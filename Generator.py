...
# VAE - has Encoder_gaussian and Decoder 
...
class Encoder_gaussian(torch.nn.Module):
    def __init__(self, param):
        ...
        self.pool_window = param['pool_window']
        self.pool_stride = param['pool_stride']
        self.len_after_max_pool = int((self.win_length - self.pool_window)/self.pool_stride) + 1
        ...
        self.max_pool_layer = torch.nn.MaxPool2d((self.pool_window, self.filt_w), stride=(self.pool_stride, 1), return_indices=True)
        ...
        self.fnn_layer1 = torch.nn.Linear(self.num_nodes_fnn_mod[0], self.num_nodes_fnn_mod[1])
    ...
    def forward(self, x):
        ...
        spec = torch.log(self.avg_pool_layer(out))  # B,C=ngf,H=1,W --> we need spec at this stage for loss function
        ...
        out4 = self.tanh(F.conv2d(self.tanh(out2 - out3), self.r2, padding=(2, 0)) + out3)  # Second rate filtering over residual, adding the 2 feature maps afterwards - SKIP
        ...
        out6 = self.tanh(F.conv2d(self.tanh(out4 - out5), self.s2, padding=(2, 0)) + out5)  # Second scale filtering over residual, adding the 2 feature maps afterwards - SKIP
...
class Decoder(torch.nn.Module):
    def __init__(self, param):
        ...
        self.fnn_layer3 = torch.nn.Linear(self.num_nodes_mean_var_mod, self.num_nodes_fnn_mod[2])
        self.fnn_layer4 = torch.nn.Linear(self.num_nodes_fnn_mod[2], self.num_nodes_fnn_mod[3]*self.len_after_max_pool)
        self.tanh = torch.nn.Tanh()
        self.max_unpool_layer = torch.nn.MaxUnpool2d((self.pool_window, self.filt_w), stride=(self.pool_stride, 1))
        self.output_layer = torch.nn.ConvTranspose2d(self.ngf, self.output_nc, kernel_size=(self.filt_h, self.filt_w), padding=(self.padding, 0), bias=False)
        
    def forward(self, x, indices):
        # print(x.shape)  # B*ngf, latent
        out = (self.tanh(self.fnn_layer3(x))) # B*ngf, fnn_node
        out_spec = torch.exp(out).contiguous().view(self.batch_size, self.ngf, -1) # [B, ngf, patch_len] we need spec at this stage for loss function
        ...
        out = out.permute(0,3,1,2).contiguous().view(self.batch_size*self.patch_length, self.ngf, -1, 1)  # [B*patch_len, ngf, patch_len, 1]
        # Transposed Convolution
        out = (self.output_layer(out))  # [B*patch_len, 1, win_len, 1]
...
class VAE(torch.nn.Module):
    ...
    def forward(self, inp):
        ...
        return E_conv_out, h_enc, D_deconv_in, out.view(inp.shape)  # The representations at spectrogram level in Encoder and Decoder (E_conv_out and D_deconv_in) are 
                                                              # also used for L2 norm in the lost function, L1 norm of Encoder output (h_enc), and L2 norm of out and inp.
